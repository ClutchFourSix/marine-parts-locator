...
function renderImageSummary(result) {
    ...
    <article class="panel listing-card result-card ${item.rank === 1 ? 'best-match' : ''}">
      <div class="result-card-layout">
        <div class="result-image-wrap">
          ${item.image_url ? `<img class="result-image" src="${item.image_url}" alt="${item.title}" loading="lazy" />` : `<div class="result-image-placeholder">Image unavailable</div>`}
        </div>
...
function renderImageAlternatives(result) {
...
function renderImageDisclaimer(result) {
...
document.addEventListener('DOMContentLoaded', () => {
  renderImageSummary(sampleResultWithImages);
  renderImageResults(sampleResultWithImages);
  renderImageAlternatives(sampleResultWithImages);
  renderImageDisclaimer(sampleResultWithImages);
});
